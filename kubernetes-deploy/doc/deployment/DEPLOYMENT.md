# Deployment Guide (Kubernetes RKE2)
...
Once all pods transition to `Running` and Readiness probes pass check the Ingress domains:

* **Web Portal (Nginx)**: `https://botanical.example.com/`
* **R-Studio Backend**: `https://botanical.example.com/rstudio-inner/`

## 5. Maintenance

* **Pod Logs**: `kubectl logs -f deployment/rstudio -c rstudio -n botanical` (Review the sidecar logs using `-c winbind-sidecar`).
* **Update Configs**: If `configmaps.yaml` or `env/.env.prd` is updated, simply re-run `./scripts/deploy_k8s.sh`. Kustomize's `configMapGenerator` will force rolling restarts on the pods.
