...
# ... (install_sssd_krb_packages, join_ad_domain_realm, generate_sssd_conf_from_template, configure_sssd_conf, etc. are unchanged from the previous version) ...
...
# --- NEW AND ENHANCED TEST FUNCTIONS ---

test_machine_keytab() {
    log "Testing Machine Kerberos Keytab (/etc/krb5.keytab)..."
    local keytab_file="/etc/krb5.keytab"
    if [[ ! -f "$keytab_file" ]]; then
        log "ERROR: Keytab file '$keytab_file' not found. The 'realm join' process likely failed."
        return 1
    fi
    log "Keytab file found. Listing principals (should include HOST/ and computer account names):"
    if ! run_command "klist -kte"; then
        log "ERROR: Command 'klist -kte' failed, or keytab is empty/unreadable."
        return 1
    fi
    log "Keytab test passed. The machine has its own Kerberos credentials."
    return 0
}

test_sssd_service_status() {
    log "Checking detailed SSSD service status..."
    # --no-pager prevents interactive mode, -l shows full log lines
    if ! run_command "systemctl status -l --no-pager sssd.service"; then
        log "Warning: 'systemctl status' command exited with an error, indicating SSSD is likely not in a healthy 'active (running)' state."
        return 1
    fi
...
test_sssd_access_control() {
    ...
    if ! command -v pamtester &>/dev/null; then log "pamtester not found."; return 1; fi
    
    local allowed_groups="${SIMPLE_ALLOW_GROUPS:-${DEFAULT_SIMPLE_ALLOW_GROUPS}}"
    if [[ -z "$allowed_groups" ]]; then
        log "INFO: 'simple_allow_groups' is not set in the configuration. This test is not applicable."
        return 0
    fi
    log "Access is currently restricted to members of: $allowed_groups"

    local use_fqn_effective="${USE_FQNS:-$DEFAULT_USE_FQNS}"
    local user_format_example="ad_user"; if [[ "$use_fqn_effective" == "true" ]]; then user_format_example="user@${AD_DOMAIN_LOWER:-$DEFAULT_AD_DOMAIN_LOWER}"; fi

    local allowed_user; read -r -p "Enter a username who IS IN an allowed group [e.g., $user_format_example]: " allowed_user
    if [[ -n "$allowed_user" ]]; then
        ...
        if run_command "pamtester --verbose rstudio \"$allowed_user\" authenticate"; then
            log "SUCCESS: Allowed user '$allowed_user' was authenticated as expected."
        else
            log "FAILURE: Allowed user '$allowed_user' FAILED authentication. This is unexpected. Check SSSD logs and group memberships."
        fi
    fi

    local disallowed_user; read -r -p "Enter a username who IS NOT in an allowed group [e.g., $user_format_example]: " disallowed_user
    if [[ -n "$disallowed_user" ]]; then
        log "Testing DISALLOWED user '$disallowed_user' (should fail)..."
        # We expect this command to fail, so we check for a non-zero exit code.
        if ! run_command "pamtester --verbose rstudio \"$disallowed_user\" authenticate"; then
            log "SUCCESS: Disallowed user '$disallowed_user' was correctly denied access as expected."
        else
            log "FAILURE: Disallowed user '$disallowed_user' was able to authenticate. This is unexpected. Check 'simple_allow_groups' in sssd.conf and the user's group memberships."
        fi
    fi
    return 0
}
...
view_sssd_logs() {
    log "Displaying last 50 lines of SSSD log (/var/log/sssd/sssd.log)..."
    log "Use Ctrl+C to stop tailing."
    run_command "tail -n 50 -f /var/log/sssd/sssd.log"
}

test_kerberos_ticket() {
    log "Testing Kerberos ticket acquisition (kinit)..."
    local test_user; local test_user_kinit_default_example="user@${AD_DOMAIN_UPPER:-$DEFAULT_AD_DOMAIN_UPPER}"
    read -r -p "Enter an AD UPN to test kinit [e.g., ${test_user_kinit_default_example}]: " test_user
    if [[ -z "$test_user" ]]; then log "Skipping test."; return 0; fi
    log "Attempting kinit for $test_user (password prompt will follow)."
    if run_command "kinit \"$test_user\""; then log "kinit successful."; run_command "klist"; run_command "kdestroy"; else log "ERROR: kinit failed."; return 1; fi
    return 0
}

test_user_lookup() {
    log "Testing user information lookup (getent, id)..."
    local test_user; local use_fqn_effective="${USE_FQNS:-$DEFAULT_USE_FQNS}"; local test_user_lookup_example="ad_username"
    ...
    log "INFO: 'getent passwd' successful. Now checking 'id'. Please verify group memberships in the output below."
    if ! run_command "id \"$test_user\""; then log "ERROR: 'id' failed."; return 1; fi
...
test_rstudio_pam_integration() {
    ...
    if [[ -z "$test_user_pam" ]]; then log "Skipping test."; return 0; fi
    ...
    if run_command "pamtester --verbose rstudio \"$test_user_pam\" authenticate acct_mgmt"; then
        ...
        log "INFO: If this was the user's first login, check if their home directory was created at the path defined by 'fallback_homedir' (e.g., ${DEFAULT_FALLBACK_HOMEDIR_TEMPLATE/\%u/$test_user_pam})."
    else
        log "RStudio PAM test FAILED."; return 1
    fi
    return 0
}
...
main_sssd_kerberos_menu() {
    ...
    while true; do
        ...
        printf "1. Full SSSD/Kerberos Setup (Time Sync, Pkgs, Join, Config)\n"
        ...
        case $choice in
            ...
            4) backup_config && configure_sssd_conf && configure_nsswitch && configure_pam && restart_and_verify_sssd ;;
            5) restart_and_verify_sssd ;;
            V1|v1) test_sssd_service_status ;;
            V2|v2) test_machine_keytab ;;
            V3|v3) test_kerberos_ticket ;;
            V4|v4) test_user_lookup ;;
            V5|v5) test_sssd_access_control ;;
            V6|v6) test_rstudio_pam_integration ;;
