# ============================================================
# BLOCK 1 OPTIMIZED - AOH TO RIJ FOR PRIORITIZR (ENHANCED)
# VERSIONE COMPLETA AGGIORNATA DOPO PREFLIGHT CON RECUPERO E OTTIMIZZAZIONI
# ============================================================
#
# Obiettivo:
...
#   1. Recupero da Interruzioni: Lo script ora gestisce in modo più robusto
#      le interruzioni (es. crash, riavvii del sistema). Se un calcolo viene
#      interrotto, può riprendere automaticamente dall'ultimo chunk completato
#      senza dover ricominciare da capo, grazie al salvataggio intermedio dei
#      risultati e dei log per ogni chunk. Questo evita la perdita di lavoro
#      e riduce drasticamente i tempi di elaborazione in caso di problemi.
#   2. Gestione Ottimizzata dello Spazio di Scratch Locale: Per migliorare
#      le prestazioni e prevenire problemi di memoria o spazio su disco,
#      lo script indirizza i file temporanei generati da 'terra' e 'GDAL'
#      verso la directory locale '/Rtmp/biome_gianfranco.samuele2'.
#      Inoltre, assicura una pulizia più rigorosa di questi file temporanei
#      al termine di ogni chunk di elaborazione, anche in caso di errori,
#      liberando risorse preziose.
#   3. Efficienza e Stabilità: Le impostazioni di thread per BLAS e GDAL,
#      insieme alla gestione della memoria di 'terra', sono state mantenute
#      e ottimizzate per garantire un'elaborazione parallela efficiente e
#      stabile, minimizzando il rischio di contesa delle risorse su sistemi
#      condivisi.
#
#   Questi miglioramenti non cambiano la logica fondamentale del calcolo
#   ma rendono il processo più affidabile, veloce e gestibile, permettendo
#   ai ricercatori di concentrarsi sui risultati piuttosto che sulla gestione
#   tecnica dei processi lunghi.
#
...
#   - usa /Rtmp/biome_gianfranco.samuele2 per file temporanei e pulizia rigorosa
#
# Output principali:
...
# Input and final output base directory
...
# Main output directory, derived from final_base_dir for a single output location
out_dir <- file.path(final_base_dir, "prioritizr_all_species_30arcmin")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# Directories for chunk-specific outputs
...
# Temporary directory for terra, located within the scratch_root
tmp_root <- file.path(scratch_root, "terra_tmp_prioritizr_all_parallel")
...
# Run configuration
# n_workers <- 1L # For single-threaded testing
# chunk_size <- 1L # For single-threaded testing

n_workers <- 10L
chunk_size <- 5L

# TRUE = delete previous outputs and start fresh.
overwrite_previous_outputs <- TRUE

# FALSE = attempt to resume from existing logs if they exist.
resume_from_existing_logs <- FALSE

# Rule for identifying AOH cells.
presence_rule <- "positive"

# Mode for calculating 'amount'.
amount_mode <- "count"

# Initial check on a few rasters.
run_preflight_check <- TRUE
n_preflight_files <- 10L

# Full run on all species.
run_limited_species_test <- FALSE
# n_species_test <- 500L # For limited testing
n_species_test <- 23000L

# Disabled as limited tests and multiple runs have been performed.
...
process_chunk <- function(this_chunk_id, species_dt, template_info, tmp_root, presence_rule, amount_mode) {
    ...
    # Set terra options for this worker
    terraOptions(
        memfrac = 0.25,
        memmax = 8, # Assuming 8GB RAM is available for this worker
        tempdir = worker_tmp,
        todisk = TRUE,
        progress = 0
    )

    # Ensure temp files are cleaned up even if an error occurs within this chunk's processing
    # This is crucial for preventing disk space issues and ensuring recovery.
    on.exit(
        {
            # Clean up temporary files created by terra for this worker
            terra::tmpFiles(remove = TRUE)
            # Clean up the worker-specific temp directory
            if (dir.exists(worker_tmp)) {
                unlink(worker_tmp, recursive = TRUE, force = TRUE)
            }
            gc(verbose = FALSE)
        },
        add = TRUE
    ) # add = TRUE ensures this doesn't overwrite other on.exit calls
    ...
    for (i in seq_len(nrow(chunk_species))) {
        ...
        if (i %% 10 == 0) {
            # Clean up temporary files periodically within a chunk
            terra::tmpFiles(remove = TRUE)
            gc(verbose = FALSE)
        }
    }
