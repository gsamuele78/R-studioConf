...
# block1_aoh_to_rij_local_scratch.R
# ============================================================
# BLOCK 1 OPTIMIZED - AOH TO RIJ FOR PRIORITIZR
# VERSIONE AGGIORNATA CON SCRATCH LOCALE SU /Rtmp
# ============================================================
#
# Obiettivo:
...
#
# Spiegazione pratica "per botanici":
#   - /nfs = spazio condiviso di rete. È il posto giusto per:
#       1) conservare i raster originali
#       2) pubblicare i risultati finali da tenere
#     Però è spesso più lento quando si leggono/scrivono moltissimi file
#     temporanei o tanti piccoli file intermedi.
#
#   - /Rtmp = disco locale del nodo di calcolo. Non è pensato come archivio
#     finale, ma è molto più veloce per:
#       1) file temporanei di terra
#       2) chunk RIJ intermedi
#       3) log intermedi
#       4) copie locali dei raster del chunk corrente
#
#   Strategia di questa versione:
#     1) i raster AOH originali restano su /nfs
#     2) per ogni chunk si copiano localmente su /Rtmp solo i raster
#        necessari in quel momento
#     3) tutti i passaggi intermedi vengono eseguiti su /Rtmp
#     4) alla fine si copiano su /nfs solo i risultati finali
#
#   In pratica:
#     /Rtmp serve per "lavorare velocemente"
#     /nfs serve per "conservare e condividere il risultato"
#
...
#   - usa scratch locale su /Rtmp per velocizzare gli step intermedi
#   - copia su /nfs solo gli output finali
#   - opzionalmente permette un benchmark iniziale su 50-100 specie
#
# Output finali su /nfs:
...
input_base_dir <- "/nfs/home/gianfranco.samuele2/test_Michele/rabinowitz/Area of Habitat"
final_base_dir <- "/nfs/home/gianfranco.samuele2/test_Michele/rabinowitz/Area of Habitat"
scratch_root <- "/Rtmp/biome_gianfranco.samuele2"

aoh_dir <- file.path(input_base_dir, "AOH_orchids")
final_out_dir <- file.path(final_base_dir, "prioritizr_all_species_30arcmin")
...
work_dir <- file.path(
    scratch_root,
    #paste0("prioritizr_all_species_30arcmin_", run_id)
    "prioritizr_all_species_30arcmin"
)

chunk_dir <- file.path(work_dir, "rij_chunks")
log_dir <- file.path(work_dir, "rij_chunk_logs")
tmp_root <- file.path(work_dir, "terra_tmp")
...
# File di lavoro locali su /Rtmp (veloci per gli step intermedi)
species_file <- file.path(work_dir, "species_table_all.csv")
rij_file <- file.path(work_dir, "rij_species_cell_30arcmin_AOH_direct_all.csv")
log_file <- file.path(work_dir, "AOH_direct_mapping_log_all.csv")
planning_units_file <- file.path(work_dir, "planning_units_30arcmin_all.csv")
chunk_summary_file <- file.path(work_dir, "chunk_processing_summary_all.csv")
species_signature_file <- file.path(work_dir, "species_table_signature.txt")
no_rij_file <- file.path(work_dir, "species_without_rij_rows.csv")
preflight_file <- file.path(work_dir, "AOH_preflight_check_first_rasters.csv")

# File finali su /nfs (da conservare e condividere)
final_species_file <- file.path(final_out_dir, "species_table_all.csv")
final_rij_file <- file.path(final_out_dir, "rij_species_cell_30arcmin_AOH_direct_all.csv")
final_log_file <- file.path(final_out_dir, "AOH_direct_mapping_log_all.csv")
final_planning_units_file <- file.path(final_out_dir, "planning_units_30arcmin_all.csv")
final_chunk_summary_file <- file.path(final_out_dir, "chunk_processing_summary_all.csv")
final_species_signature_file <- file.path(final_out_dir, "species_table_signature.txt")
final_no_rij_file <- file.path(final_out_dir, "species_without_rij_rows.csv")
final_preflight_file <- file.path(final_out_dir, "AOH_preflight_check_first_rasters.csv")
...
# TRUE = la run usa una scratch locale nuova e, a fine run riuscita,
# sovrascrive gli output finali su /nfs.
overwrite_previous_outputs <- TRUE

# Con scratch su /Rtmp non conviene assumere che i log locali esistano ancora
# dopo reboot/cleanup del nodo. Quindi, per default, niente resume.
...
# Per benchmark iniziale (es. 50-100 specie), impostare TRUE.
run_limited_species_test <- FALSE
n_species_test <- 100L

# Disattivato perché questa run lavora in una scratch locale nuova.
check_species_signature <- FALSE

# TRUE = dopo il sync finale elimina la scratch locale.
# Lasciato FALSE per permettere controlli/debug dopo la run.
cleanup_local_scratch_at_end <- FALSE
...
stage_one_file <- function(src, dst_dir) {
    if (!file.exists(src)) {
        stop("File sorgente non trovato per staging locale: ", src)
    }
    ...
    if (file.exists(dst)) {
        ...
        if (
            !is.na(src_info$size) &&
                !is.na(dst_info$size) &&
                identical(as.numeric(src_info$size), as.numeric(dst_info$size))
        ) {
            return(dst)
        }
    ...
    if (!isTRUE(ok) || !file.exists(dst)) {
        stop("Copia locale del raster fallita: ", src, " -> ", dst)
    }
    ...
    if (
        ...
        stop("Copia locale incompleta del raster: ", src, " -> ", dst)
    }

    dst
}

copy_to_final <- function(src, dst) {
    if (!file.exists(src)) {
        return(FALSE)
    }
    ...
    if (!isTRUE(ok) || !file.exists(dst)) {
        stop("Copia verso NFS fallita: ", src, " -> ", dst)
    }
    ...
    if (
        ...
        stop("Copia verso NFS incompleta: ", src, " -> ", dst)
    }
...
cat("Input raster su /nfs:", aoh_dir, "\n")
cat("Scratch locale veloce su /Rtmp:", work_dir, "\n")
cat("Output finali pubblicati su /nfs:", final_out_dir, "\n")
cat(
    "\nNota pratica:\n",
    "- /Rtmp è locale al nodo, quindi è più veloce per chunk, log e file temporanei.\n",
    "- /nfs è condiviso in rete, quindi lo usiamo per leggere gli input originali\n",
    "  e per salvare solo i risultati finali da conservare.\n",
    sep = ""
)
...
if (run_limited_species_test) {
    cat(
        "\nATTENZIONE: benchmark/run limitata a",
        min(n_species_test, nrow(species_dt)),
        "specie.\n"
    )
...
if (run_preflight_check) {
    ...
    cat("\nPreflight salvato localmente in:\n", preflight_file, "\n")
...
old_signature <- read_species_signature(final_species_signature_file)

if (overwrite_previous_outputs) {
    cat(
        "\nModalità overwrite attiva:\n",
        "- si usa una scratch locale nuova per questa run\n",
        "- gli output finali su /nfs verranno sovrascritti SOLO alla fine,\n",
        "  dopo una run completata con successo\n",
        sep = ""
    )
    old_signature <- NULL
}
...
cat("\nSpecies table salvata localmente in:\n", species_file, "\n")
...
process_one_aoh_worker <- function(
  sp_id,
  sp_name,
  f,
  input_file_label,
...
process_chunk <- function(this_chunk_id, species_dt, template_info, tmp_root, presence_rule, amount_mode) {
    ...
    input_stage_dir <- file.path(worker_tmp, "aoh_inputs")
    dir.create(input_stage_dir, recursive = TRUE, showWarnings = FALSE)
    ...
    for (i in seq_len(nrow(chunk_species))) {
        ...
        f_nfs <- chunk_species$input_file[i]

        f_local <- tryCatch(
            stage_one_file(f_nfs, input_stage_dir),
            error = function(e) e
        )

        if (inherits(f_local, "error")) {
            rij_list[[i]] <- NULL
            log_list[[i]] <- data.table(
                species_id = as.integer(sp_id),
                species_name = sp_name,
                input_file = f_nfs,
                status = "input_stage_copy_error",
                minutes = NA_real_,
                n_cells_total = NA_integer_,
                n_cells_na = NA_integer_,
                n_cells_non_na = NA_integer_,
    ...
    terra::tmpFiles(remove = TRUE)
    unlink(input_stage_dir, recursive = TRUE, force = TRUE)
    gc(verbose = FALSE)
...
cat(
    "Modalità I/O:\n",
    "- input originari: /nfs\n",
    "- copie locali del chunk corrente: /Rtmp\n",
    "- file temporanei di terra: /Rtmp\n",
    "- chunk e log intermedi: /Rtmp\n",
    "- risultati finali: copiati su /nfs a fine run\n",
    sep = ""
)
...
cat("RIJ finale salvato localmente in:\n", rij_file, "\n")
...
if (length(species_without_rij) > 0) {
    ...
    cat("Lista specie senza RIJ salvata localmente in:\n", no_rij_file, "\n")
}
...
cat("Planning units salvate localmente in:\n", planning_units_file, "\n")

# ------------------------------------------------------------
# 15. SYNC FINAL OUTPUTS TO NFS
# ------------------------------------------------------------

cat("\nCopio i risultati finali su /nfs...\n")
cat(
    "Spiegazione pratica:\n",
    "- tutti gli step intermedi sono stati eseguiti in locale su /Rtmp per velocità\n",
    "- ora si pubblicano su /nfs solo i file finali da conservare\n",
    sep = ""
)

copy_to_final(species_file, final_species_file)
copy_to_final(rij_file, final_rij_file)
copy_to_final(log_file, final_log_file)
copy_to_final(planning_units_file, final_planning_units_file)
copy_to_final(chunk_summary_file, final_chunk_summary_file)
copy_to_final(species_signature_file, final_species_signature_file)

if (file.exists(preflight_file)) {
    copy_to_final(preflight_file, final_preflight_file)
} else if (file.exists(final_preflight_file)) {
    file.remove(final_preflight_file)
}

if (file.exists(no_rij_file)) {
    copy_to_final(no_rij_file, final_no_rij_file)
} else if (file.exists(final_no_rij_file)) {
    file.remove(final_no_rij_file)
}

cat("\nRisultati finali copiati in:\n", final_out_dir, "\n")
...
cat("\n================ FINAL SUMMARY ================\n")
cat("Scratch locale veloce (/Rtmp):\n", work_dir, "\n")
cat("Species table finale (/nfs):\n", final_species_file, "\n")
cat("RIJ finale (/nfs):\n", final_rij_file, "\n")
cat("Planning units finali (/nfs):\n", final_planning_units_file, "\n")
cat("Log finale (/nfs):\n", final_log_file, "\n")
cat("Chunk summary finale (/nfs):\n", final_chunk_summary_file, "\n")
cat("Preflight finale (/nfs):\n", final_preflight_file, "\n")
cat("Presence rule:\n", presence_rule, "\n")
cat("Amount mode:\n", amount_mode, "\n")
cat(
    "Messaggio chiave:\n",
    "- /Rtmp = lavoro locale veloce per passaggi intermedi\n",
    "- /nfs = archivio condiviso per i risultati finali\n",
    sep = ""
)
cat("================================================\n")

if (cleanup_local_scratch_at_end) {
    cat("\nPulizia scratch locale finale...\n")
    unlink(work_dir, recursive = TRUE, force = TRUE)
    cat("Scratch locale rimossa:", work_dir, "\n")
}
