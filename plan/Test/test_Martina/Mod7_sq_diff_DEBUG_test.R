...
# ══════════════════════════════════════════════════════════════════════════════
# CAPTURE SCRIPT START TIME — single source of truth for all timestamps
# ══════════════════════════════════════════════════════════════════════════════
SCRIPT_START_TIME <- Sys.time()

# Display format (with colons) — used in console output and worker logs
TIMESTAMP_DISPLAY <- format(SCRIPT_START_TIME, "%Y-%m-%d %H:%M:%S")

# Folder-safe format (underscores/hyphens) — used for directory names
TIMESTAMP_FOLDER <- format(SCRIPT_START_TIME, "%Y-%m-%d_%H-%M")

# to export in cluster use list
...
saveRDS(TIMESTAMPS, file = "TIMESTAMPS.rds")
...
clusterExport(N_cluster, varlist = c(
    ...
    "N_tot_iter", "N_burn", "N_thin", "TIMESTAMPS"
), envir = environment())
    ...
    result <- tryCatch(
        {
            ...
            cat("Worker log directory:", worker_log_dir, "\n")
            cat("User tmp root:", user_tmp_root, "\n")
...
saveRDS(chain_output, file = "Lagorai/mod7_HMC_4ch55k50k1T_200226_201pl_Lagorai.rds")

# ══════════════════════════════════════════════════════════════════════════════
# TRACEPLOTS — Organized output in plots/YYYY-MM-DD_HH-MM/{pdf,png}/
# ══════════════════════════════════════════════════════════════════════════════
...
# Create timestamp-based run folder: plots/YYYY-MM-DD_HH-MM/{pdf,png}/
# Uses TIMESTAMP_FOLDER captured at script start for consistency
plots_base <- file.path(getwd(), "plots", TIMESTAMP_FOLDER)
pdf_dir <- file.path(plots_base, "pdf")
png_dir <- file.path(plots_base, "png")
...
cat("\n")
cat("================================================================================\n")
cat("GENERATING TRACEPLOTS\n")
cat("================================================================================\n")
cat("Output directory:", plots_base, "\n")
cat("  PDF output: ", pdf_dir, "/Rplots.pdf\n")
cat("  PNG output: ", png_dir, "/Rplot*.png\n")
cat("================================================================================\n")
cat("\n")

# ── PDF output (default name: Rplots.pdf) ─────────────────────────────────────
cat("[1/3] Generating PDF traceplots...\n")
old_wd <- setwd(pdf_dir)
tryCatch(
    {
        # pdf = TRUE tells MCMCvis to use PDF device (one file for all plots)
        MCMCvis::MCMCtrace(lapply(chain_output, function(i) i[["samples"]]), pdf = TRUE)
        cat("  PDF saved:", file.path(pdf_dir, "Rplots.pdf"), "\n")
    },
    error = function(e) {
...
# ── PNG output (default names: Rplot001.png, Rplot002.png, ...) ──────────────
cat("[2/3] Generating PNG traceplots...\n")
old_wd <- setwd(png_dir)
old_dev <- getOption("device")
options(device = png) # Set default device to PNG
tryCatch(
    {
        # pdf = FALSE tells MCMCvis to use current device (PNG) — creates multiple files
        MCMCvis::MCMCtrace(lapply(chain_output, function(i) i[["samples"]]), pdf = FALSE)
        png_files <- list.files(png_dir, pattern = "^Rplot.*\\.png$", full.names = TRUE)
        cat("  PNG files saved:", length(png_files), "files in", png_dir, "\n")
    },
    error = function(e) {
        cat("  PNG generation failed:", e$message, "\n")
    }
)
options(device = old_dev) # Restore original device option
setwd(old_wd)

# ── Traditional traceplot (also to PNG in the same directory) ─────────────────
cat("[3/3] Generating traditional traceplots...\n")
old_wd <- setwd(png_dir)
tryCatch(
    {
        png(filename = "traceplot_traditional.png", width = 1920, height = 1080, res = 150)
        traceplot(out_list)
        dev.off()
        cat("  Traditional traceplot saved:", file.path(png_dir, "traceplot_traditional.png"), "\n")
    },
    error = function(e) {
        cat("  Traditional traceplot failed:", e$message, "\n")
    }
)
setwd(old_wd)
...
cat("================================================================================\n")
cat("All plots saved to:", plots_base, "/\n")
cat("  - PDF: ", pdf_dir, "/Rplots.pdf\n")
cat("  - PNG: ", png_dir, "/Rplot*.png\n")
cat("================================================================================\n")
# nolint end
