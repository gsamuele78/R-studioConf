...
constants <- list(n = Smp_size, p = N_col_XforGDM, x = X_for_GDM, n_loc = N_sites, 
                  R_inv = R_inv, zeros = rep(0, N_sites), row_ind = row_ind, col_ind = col_ind)

#create data for nimble model

data_mod <- list(log_V = ifelse(Obs_Z == 1, NA, log(Obs_Z)),
                 censored = 1*(Obs_Z == 1),
                 c = rep(0, constants$n))
...
init_list <- replicate(n = 4, expr = {
  list(beta_0 = lm_out$par[1] + rnorm(n = 1, mean = 0, sd = .1),
       ...
       psi = lm_out$par[-(1:(N_col_XforGDM + 1))] + rnorm(n = N_sites, mean = 0, sd = .0005)) #changed .00001 to .01 #changed .01 to .005
}, simplify = F)
...
run_MCMC_allcode <- function(seed, data, code, constants, inits, niter, nburnin, thin = 1, smmry = T, wid_aic = F) {
  ...
  results <- runMCMC(CmyModel$myMCMC, niter = niter, nburnin = nburnin, thin = thin, nchains = 1, inits = inits[[seed]],
...
chain_output <- parLapply(cl = N_cluster, X = 1:4, 
