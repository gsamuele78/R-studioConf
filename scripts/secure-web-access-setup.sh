...
# VERSION 14.1: VICTORIOUS. Separates apt install commands for robustness.
...
install_services() {
    ...
    run_command "Force configure all pending packages" "dpkg --configure -a"
    run_command "Fix broken dependencies" "apt-get -f install -y"
    
    # ### DEFINITIVE FIX: Install packages one by one to avoid apt quirks ###
    log "INFO" "Installing prerequisite packages..."
    run_command "Update package lists" "apt-get -y update"
    
    log "INFO" "Installing ttyd. Apt output will follow:"
    if ! DEBIAN_FRONTEND=noninteractive apt-get -y install ttyd; then 
        handle_error $? "Failed to install ttyd."; return 1; 
    fi

    log "INFO" "Installing curl. Apt output will follow:"
    if ! DEBIAN_FRONTEND=noninteractive apt-get -y install curl; then 
