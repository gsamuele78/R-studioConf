...
if [[ "$*" == *"--mail"* ]]; then
    ...
    # Pre-defining vars referenced by legacy r_orphan_cleanup.conf so that
    # `source` under `set -u` does not abort if the conf was generated by an
    # older deploy that lacks newer placeholders (e.g. MAIL_DOMAINS_USER added
    # for multi-domain mail support).
    : "${MAIL_DOMAINS_USER:=}"
    : "${MAIL_DOMAIN:=}"
    # shellcheck source=/dev/null
    source "${CONF_FILE}"
    # Re-assert defaults in case the conf set them to literal envsubst tokens
    # (e.g. the string '${MAIL_DOMAINS_USER}' left unsubstituted by an old run).
    case "${MAIL_DOMAINS_USER}" in
        *'${'*) MAIL_DOMAINS_USER="" ;;
    esac
